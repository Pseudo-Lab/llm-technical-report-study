...
- [ ] DSA가 MLA latent KV 전체에 dense attention하지 않고 indexer top-k만 core attention에 보내며, dense warm-up과 sparse training으로 selector를 어떻게 학습하는지 설명할 수 있다.
  ...
  - **함께 읽을 자료와 범위:** [W09 개념 경로 §1](../../background/w09.md#dsa) → V3.2이 직접 연결한 [DeepSeek-V2 v5 §2.1.1–§2.1.4](https://arxiv.org/pdf/2405.04434v5)와 [MQA v1 §2.4](https://arxiv.org/pdf/1911.02150v1). DSA의 selection과 MQA의 KV-sharing을 다른 축으로 그린다.
...
- [ ] V3.2가 rollout을 여러 update에 재사용하고 MoE/top-p sampling을 쓸 때, unbiased KL·off-policy sequence masking·Keep Routing·Keep Sampling Mask로 GRPO를 어떻게 안정화하는지 설명할 수 있다.
  ...
  - **함께 읽을 자료와 범위:** [W09 개념 경로 §2](../../background/w09.md#scaling-grpo) → GRPO 원전 [DeepSeekMath v3 §4.1.1–§4.1.3](https://arxiv.org/pdf/2402.03300v3) → V3.2 §3.1. “mask”가 모든 off-policy sample을 버리는 것인지, 어떤 조건의 negative sequence만 가리는지 식 (8)–(9)에서 확인한다.
